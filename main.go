...
func listKeysAction(ctx *cli.Context) error {
	...
	if keysDir == "" {
		return cli.NewExitError("Required option 'keypairsdir' not provided. Use the '--help' option for more information.", 3)
	...
	if err != nil {
		...
		return cli.NewExitError(fmt.Sprintf("Key listing error %v", keysDir), 2)
	}
...
func generateNewKeysAction(ctx *cli.Context) error {
	...
	if outputDir == "" {
		return cli.NewExitError("Required option 'outputdir' not provided. Use the '--help' option for more information.", 3)
		...
		if err != nil {
			...
			return cli.NewExitError(fmt.Sprintf("Unable to use output directory '%v'", outputDir), 3)
		}
	}

	outputCheck, err := os.Stat(outputDir)
	if err != nil {
		return cli.NewExitError(fmt.Sprintf("Unable to stat given directory %v", outputDir), 3)
	}
	...
	if err != nil {
		...
		return cli.NewExitError("Failed to generate new keys", 2)
	}
...
func signAction(ctx *cli.Context) error {
	...
	if key == "" {
		return cli.NewExitError("Required option 'privatekey' not provided. Use the '--help' option for more information.", 3)
	}
	...
	if err != nil {
		...
		return cli.NewExitError("Failed to sign input", 2)
	}
...
func verifyAction(ctx *cli.Context) error {
	...
	if keyPath == "" {
		return cli.NewExitError("Required option 'publickey' not provided. Use the '--help' option for more information.", 3)
	}
	...
	if signaturePath == "" {
		return cli.NewExitError("Required option 'signature' not provided. Use the '--help' option for more information.", 3)
	}

	file, err := os.Open(signaturePath)
	if err != nil {
		return cli.NewExitError("Unable to open file at given signature file path.", 3)
	}
	defer file.Close()
...
func main() {
	...
	app.Commands = []cli.Command{
		...
		cli.Command{
			...
			Aliases: []string{"gk"},
			Usage:   "Generate a new private/public keypair; output both with public key wrapped in x509 certificate",
			Flags: []cli.Flag{
				cli.StringFlag{
					Name: "outputdir, d",
				...
				cli.StringFlag{
					Name:   "x509org, xo",
					Usage:  "x509 certificate Organization (O) field (preferrably a company name or other organization's name)",
					EnvVar: "RSAPSSTOOL_X509ORG",
				},
				cli.StringFlag{
					Name:   "x509cn, xcn",
					Usage:  "x509 certificate Common Name (CN) field (preferrably an email address issued by x509org",
